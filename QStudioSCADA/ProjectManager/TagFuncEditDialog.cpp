...
{
    if (!index.isValid() ||
        index.row() < 0 || index.row() >= m_FuncList.count() ||
        index.column() < 0 || index.column() >= MaxTableColumns)
        return QVariant();
    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole)
    {
        switch (index.column())
        {
            case FUNC_NAME: return pfunc->GetFuncString();
            case EVENT_TYPE: return pfunc->event;
...
bool FuncModel::setData(const QModelIndex &index,
...
bool FuncModel::insertRows(int row, int count, const QModelIndex&)
{
    ...
    for (int i=0; i<count; ++i)
        m_FuncList.insert(row, new FuncObjectItem());
    endInsertRows();
    return true;
}
    ...
    m_strProjectPath(projectPath)
{
    ...
    connect(m_funcTableView, SIGNAL(clicked(QModelIndex)), this, SLOT(FuncTableViewClicked(QModelIndex)));
...
void TagFuncEditDialog::TreeViewInit()
{
    ...
    for (int i=0; i<sFuncTypeList.count(); i++)
    {
        ...
        {
            TFuncObject *pFuncObj = m_supportFuncList.at(i);
            //qDebug()<< pFuncObj->name << "  " << pFuncObj->desc << "  " << pFuncObj->args_type;
...
void TagFuncEditDialog::GetElementProperty(PFuncObjectItem pFuncObj)
{
    ...
    //qDebug() << m_pCurFuncObj->name << m_pCurFuncObj->argList.count() << m_pCurFuncObj->event;
    ...
    QtVariantProperty *eventItem = m_pVariantManager->addProperty(QtVariantPropertyManager::enumTypeId(), tr("事件类型"));
...
* 插槽：功能函数属性值改变
*/
void TagFuncEditDialog::FuncPropertyValueChanged(QtProperty *property, const QVariant &value)
{
    if (!m_pCurFuncObj)
        return;

    if (!m_mapPropertyToId.contains(property))
        return;
...
* 获取函数名和参数
*/
bool TagFuncEditDialog::GetFuncNameArgs(QString funcString, QString &funcName, QList<TArgItem> &argList)
{
    if(funcString == "")
        return false;
    funcName = funcString.left(funcString.indexOf("("));
    QString strArgs = funcString.mid(funcString.indexOf("(") + 1, funcString.indexOf(")") - funcString.indexOf("(") - 1);
