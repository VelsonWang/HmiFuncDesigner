...
NewProjectDialog::NewProjectDialog(QWidget *parent, QString projPath)
    : QDialog(parent), ui(new Ui::NewProjectDialog), projectPath_(projPath) {
  ui->setupUi(this);
  ...
  QString iniDevFileName =
      QCoreApplication::applicationDirPath() + "/Config/device.ini";
  ...
  ConfigUtils::getCfgList(iniDevFileName, "DeviceSupportIndex", "dev",
                          slistDev);
...
void NewProjectDialog::on_btnFileDialog_clicked() {
  QString dir = QFileDialog::getExistingDirectory(
      this, tr("选择工程目录"), "C:/", QFileDialog::ShowDirsOnly);
...
void NewProjectDialog::on_btnExit_clicked() { QDialog::reject(); }

QString NewProjectDialog::GetProjectName() { return projectName_; }
...
bool NewProjectDialog::load() {
  ...
  ui->editStationNumber->setText(
      QString::number(projInfoMgr.getStationNumber()));
  ...
  ui->editPageScanPeriod->setText(
      QString::number(projInfoMgr.getPageScanPeriod()));
  ui->editDataScanPeriod->setText(
      QString::number(projInfoMgr.getDataScanPeriod()));
