...
DrawPageWin::DrawPageWin(QWidget *parent, const QString &itemName,
    ...
    : ChildBase(parent, itemName, projName), ui(new Ui::DrawPageWin) {
  ui->setupUi(this);
...
*  打开文件
*/
void DrawPageWin::open() { DrawListUtils::loadDrawList(szProjPath_); }
...
void DrawPageWin::createEmptyGraphpage(const QString &projPath,
                                       const QString &graphPageName, int width,
                                       int height) {
...
void DrawPageWin::ListViewUpdate() {
  ...
  QStandardItem *pNewWin =
      new QStandardItem(QIcon(":/images/drawexec.png"), tr("新建画面"));
  pNewWin->setEditable(false);
  pListDrawPageModel->appendRow(pNewWin);

  for (int i = 0; i < DrawListUtils::drawList_.count(); i++) {
    QStandardItem *pDrawPage = new QStandardItem(
        QIcon(":/images/drawexec.png"), DrawListUtils::drawList_.at(i));
