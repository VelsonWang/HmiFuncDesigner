  ...
  foreach (DBVarGroup *var, TagManager::ioDBVarGroups_.varBlockGroupList_) {
    if (text == var->m_name) {
      ...
      if (pDlg->exec() == QDialog::Accepted) {
        ...
        desfile = ProjectMgrUtils::getProjectPath(m_strProjectName) +
                  "/DevVarList-" + var->m_name + ".odb";
...
void MainWindow::VariableDeleteGroup() {
  QModelIndex ModelIndex =
      ui->treeViewProject->selectionModel()->currentIndex();
  ...
  foreach (DBVarGroup *var, TagManager::ioDBVarGroups_.varBlockGroupList_) {
    if (qTiem->text() == var->m_name) {
      ...
      QString file = "";
      file = ProjectMgrUtils::getProjectPath(m_strProjectName) +
             "/DevVarList-" + var->m_name + ".odb";
      QFile delFile(file);
      if (delFile.exists()) {
        delFile.remove();
...
void MainWindow::contextMenuEvent(QContextMenuEvent * /*event*/) {
  ...
  if (m_CurTreeViewItem == "设备变量") found = true;

  foreach (DBVarGroup *var, TagManager::ioDBVarGroups_.varBlockGroupList_) {
    if (m_CurTreeViewItem == var->m_name) found = true;
  }
  ...
  connect(pRenameGroupAct, SIGNAL(triggered()), this,
          SLOT(VariableGroupRename()));
  pMenu->addAction(pRenameGroupAct);

  QAction *pDeleteGroupAct = new QAction(tr("删除组"), this);
  pDeleteGroupAct->setStatusTip(tr("删除组"));
  connect(pDeleteGroupAct, SIGNAL(triggered()), this,
          SLOT(VariableDeleteGroup()));
  ...
  connect(pModifyGroupIDAct, SIGNAL(triggered()), this,
          SLOT(VariableModifyGroupID()));
...
void MainWindow::readSettings() {
  ...
  if (geometry.isEmpty()) {
    ...
    resize(availableGeometry.width() * 3 / 4,
           availableGeometry.height() * 3 / 4);
...
void MainWindow::on_actionNewPoject_triggered() {
  ...
  if (pNewProjectDlg->exec() == QDialog::Accepted) {
    ...
    QString szName =
        ProjectMgrUtils::getProjectNameWithOutSuffix(m_strProjectName);
...
void MainWindow::on_treeViewProject_clicked(const QModelIndex &index)
{
    ...
    QString strName = name.mid(name.lastIndexOf("/") + 1,
                               name.indexOf(".") - name.lastIndexOf("/") - 1);
...
void MainWindow::UpdateDeviceVariableTableGroup() {
  while (!pDevVariableTabList.empty()) pDevVariableTabList.takeFirst();
  // while(pDevVariable->rowCount())
  pDevVariable->removeRows(0, pDevVariable->rowCount());
  foreach (DBVarGroup *var, TagManager::ioDBVarGroups_.varBlockGroupList_) {
    QStandardItem *pDevVarTab =
        new QStandardItem(QIcon(":/images/pj_zone.png"), var->m_name);
...
void MainWindow::on_actionImportTag_triggered() {
  ...
  QString strSaveCsvFile = QFileDialog::getOpenFileName(
      this, tr("选择csv文件"), path, tr("csv file (*.csv)"));

  if (strSaveCsvFile == "") return;

  QString strCsvName = strSaveCsvFile.mid(
      strSaveCsvFile.lastIndexOf("/") + 1,
      strSaveCsvFile.indexOf(".") - strSaveCsvFile.lastIndexOf("/") - 1);
...
void MainWindow::loadRecentProjectList() {
  QString iniRecentProjectFileName =
      Helper::AppDir() + "/RecentProjectList.ini";
  QFile fileCfg(iniRecentProjectFileName);
  if (fileCfg.exists()) {
    QStringList slist;
    ConfigUtils::getCfgList(iniRecentProjectFileName, "RecentProjects",
                            "project", slist);
  ...
  for (int i = 0; i < listAct.size(); ++i) {
    ...
    if (pAct->isSeparator()) listActRemove.append(pAct);
    if (pAct->text() == tr("最近文件列表")) listActRemove.append(pAct);
  }
...
void MainWindow::updateRecentProjectList(QString newProj) {
  ...
  if (fileCfg.exists()) {
    ...
    ConfigUtils::writeCfgList(iniRecentProjectFileName, "RecentProjects",
                              "project", slist);
