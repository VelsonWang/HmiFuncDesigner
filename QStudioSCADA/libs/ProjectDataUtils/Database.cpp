...
Database::Database(const QString &dbname, const QString &user,
                   const QString &pwd, const QString &hostname, int port,
                   QObject *parent)
    : QObject(parent) {
  name_ = dbname;
  user_ = user;
...
bool Database::getRecord(const QString &table, const QStringList &keyList,
                         QStringList &valueList, const QString &expr) {
  int i, count;
  QSqlQuery query(db_);
  QString key, sql;
  ...
  try {
    ...
    LogError(
        QString("get record: %1 failed! Exception: %2").arg(table).arg(sql));
    return false;
  }
  return true;
}

bool Database::getRecord(const QString &table, const QString &key,
                         QStringList &valueList, const QString &expr) {
  QSqlQuery query(db_);
  QString sql;
  ...
  try {
    ...
    while (query.next()) valueList << query.record().value(key).toString();
  } catch (...) {
    LogError(
        QString("get record: %1 failed! Exception: %2").arg(table).arg(sql));
    return false;
...
bool Database::getRecord(const QString &table, const QString &key,
                         QString &value, const QString &expr) {
  QSqlQuery query(db_);
  QString sql;
  ...
  try {
    ...
    if (query.next()) value = query.record().value(key).toString();
  } catch (...) {
    LogError(
        QString("get record: %1 failed! Exception: %2").arg(table).arg(sql));
    return false;
...
bool Database::getRecord(const QString &table, const QStringList &keyList,
  ...
  try {
    ...
    while (query.next()) {
      ...
      if (count == valueList.count()) valueListList << valueList;
    }
  } catch (...) {
    LogError(
        QString("get record: %1 failed! Exception: %2").arg(table).arg(sql));
...
bool Database::setRecord(const QString &table, const QString &key,
                         const QString &value, const QString &expr) {
  QSqlQuery query(db_);
  QString sql;
  ...
  try {
    ...
    LogError(
        QString("set record: %1 failed! Exception: %2").arg(table).arg(sql));
    return false;
  }
  return true;
}

bool Database::setRecord(const QString &table, const QStringList &keyList,
                         const QStringList &valueList, const QString &expr) {
  int i, count;
  QSqlQuery query(db_);
  QString key, sql;
  ...
  if (!count || count != valueList.count()) {
    LogError(QString("set record: %1 failed! keylist.count != valuelist.count")
                 .arg(table));
    return false;
  }
...
bool Database::insertRecord(const QString &table, const QStringList &keyList,
                            const QStringList &valueList, const QString &expr) {
  int i, count;
  QSqlQuery query(db_);
  ...
  if (!count || count != valueList.count()) {
    LogError(
        QString("insert record: %1 failed! keylist.count != valuelist.count")
            .arg(table));
    return false;
  }
  ...
  if (expr.isEmpty())
    sql =
        QString("insert into %1(%2) values(%3)").arg(table).arg(key).arg(value);
  else
    sql = QString("insert into %1(%2) values(%3) where %4")
              .arg(table)
              .arg(key)
              .arg(value)
              .arg(expr);
  try {
    ...
    LogError(
        QString("insert record: %1 failed! Exception: %2").arg(table).arg(sql));
    return false;
  }
  return true;
}

bool Database::insertRecord(const QString &table, const QString &key,
                            const QStringList &valueList, const QString &expr) {
  QSqlQuery query(db_);
  QString value, sql;
...
bool Database::copyRecord(const QString &tableFrom, const QString &tableTo,
                          const QString &expr) {
  QSqlQuery query(db_);
  QString sql;
  if (tableFrom.isEmpty() || tableTo.isEmpty()) {
    LogError(QString("copy record: form %1 to %2 failed!")
                 .arg(tableFrom)
                 .arg(tableTo));
    return false;
  }
  ...
  if (expr.isEmpty())
    sql =
        QString("insert into %1 select * from %2 ").arg(tableTo).arg(tableFrom);
  else
    sql = QString("insert into %1 select * from %2 where %3")
              .arg(tableTo)
              .arg(tableFrom)
              .arg(expr);
...
int Database::getLastInsertId(const QString &tableName) {
  ...
  QString szSql = QString("SELECT DISTINCT last_insert_rowid() as id FROM %1")
                      .arg(tableName);
