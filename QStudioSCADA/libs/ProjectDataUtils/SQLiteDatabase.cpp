...
SQLiteDatabase::SQLiteDatabase(const QString &dbname, const QString &user,
                               const QString &pwd, const QString &hostname,
...
int SQLiteDatabase::createTable(const QString &table, QStringList fieldList,
                                QStringList typeList, const QString &index) {
  QString key = QString();
  QSqlQuery query(db_);
  int pos = 0, ret = 0;
  ...
  if (!query.next() || (query.value(0).toInt() == 0)) {
    ...
    if (!query.exec(QString("create table %1(%2)").arg(table, key))) {
      LogError(QString("create table: %1 failed! deatil: %2!")
                   .arg(table)
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (pos == -1) {
        if (!query.exec(QString("alter table %1 add column %2 %3")
          ...
          LogError(
              QString("alter table: %1 failed! deatil: Failed or Exception")
                  .arg(table));
          return 0;
        }
        ret = 2;
...
bool SQLiteDatabase::insertOrUpdateRecord(const QString &table,
  ...
  if (!count || count != valueList.count()) {
    LogError(
        QString("insert record to %1 failed! keylist.count != valuelist.count")
            .arg(table));
    return false;
  }
  ...
  sql = QString("replace into %1(%2) value(%3)").arg(table).arg(key).arg(value);
