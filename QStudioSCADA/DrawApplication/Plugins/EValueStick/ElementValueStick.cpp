...
void ElementValueStick::createPropertyList()
{
    ...
    // 滑动输入
    slideInputProperty_ = new BoolProperty(tr("滑动输入"));
    slideInputProperty_->setId(EL_SLIDE_INPUT);
    slideInputProperty_->setTrueText(tr("是"));
    slideInputProperty_->setFalseText(tr("否"));
    slideInputProperty_->setValue(slideInput_);
    propList.insert(propList.end(), slideInputProperty_);
...
void ElementValueStick::drawValueStick(QPainter *painter)
{
    ...
    if ( szScaleDir == QString("LeftToRight") || szScaleDir == QString("RightToLeft") ) {
        ...
        // 绘制滑块
        if ( slideInput_ && ((barRect.right() - barRect.left()) >= 4) ) {
            if(szScaleDir == QString("LeftToRight")) {
                barRect.setLeft(barRect.right() - 2);
                barRect.setRight(barRect.right() + 2);
            } else {
                barRect.setRight(barRect.left() + 2);
                barRect.setLeft(barRect.left() - 2);
            }

            PubTool::Draw3DFrame(painter, barRect, color3DHiLight, color3DShadow, Qt::gray);
        }
        // 绘制标尺
        ...
        // 绘制滑块
        if ( slideInput_ && ((barRect.bottom() - barRect.top()) >= 4) ) {
            if(szScaleDir == QString("TopToBottom")) {
                barRect.setTop(barRect.bottom() - 2);
                barRect.setBottom(barRect.bottom() + 2);
            } else {
                barRect.setBottom(barRect.top() + 2);
                barRect.setTop(barRect.top() - 2);
            }
...
void ElementValueStick::writeAsXml(QXmlStreamWriter &writer)
{
    ...
    writer.writeAttribute("slideInput", slideInput_?"true":"false");
...
void ElementValueStick::readFromXml(const QXmlStreamAttributes &attributes)
{
    ...
    if (attributes.hasAttribute("slideInput")) {
        QString value = attributes.value("slideInput").toString();
        slideInput_ = false;
        if(value == "true") {
            slideInput_ = true;
        }
