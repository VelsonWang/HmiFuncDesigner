...
void S7_200::setPort(IPort *pPort) {
    if(pPort != nullptr)
        iFacePort_ = pPort;
}


IPort *S7_200::getPort() {
    return iFacePort_;
}


/**
 * @brief S7_200::isCanWrite
 * @details 判断该变量定义属性是否可以写
 * @param pTag 设备变量
 * @return false-不可写，true-可写
 */
bool S7_200::isCanWrite(IOTag* pTag) {
...
/**
 * @brief S7_200::writeData
 * @details 写一个变量值到plc设备
 * @param pTag 设备变量
 * @return 0-失败, 1-成功
 */
int S7_200::writeData(IOTag* pTag) {
    ...
    // 被写数据指针
    quint8 *pVarVailableData = (quint8 *)pDBTagObject->GetWriteDataBytes().data();
    ...
    if(getCpuMem(pTag->GetRegisterArea()) == CM_T) {
        ...
        if(*(quint8 *)pVarVailableData)
            writeDataBuffer_[0] = 0x01;
        else
            writeDataBuffer_[0] = 0x00;
    ...
    if(getPort() != Q_NULLPTR)
        getPort()->write(aDatabuf, 37+wTemp, 1000);
    ...
    if(readDataBuffer_[0] == 0xE5 || readDataBuffer_[0] == 0x10) {
        ...
        do
        {
            ...
            // 序列化写确认包
            if(getPort() != Q_NULLPTR)
                getPort()->write((quint8 *)&stWriteAck, 6, 1000);

            memset(readDataBuffer_, 0, 512);
            QThread::msleep(WRITE_DELAY);

            if(getPort() != Q_NULLPTR)
                resultlen = getPort()->read(readDataBuffer_, 1, 3000);
        ...
        if(getPort() != Q_NULLPTR)
            resultlen = getPort()->read(readDataBuffer_ + 1, WRITE_REPLAY_LEN - 1, 3000);
...
/**
 * @brief S7_200::isCanRead
 * @details 判断该变量定义属性是否可以读
 * @param pTag 设备变量
 * @return false-不可读，true-可读
 */
bool S7_200::isCanRead(IOTag* pTag) {
    if(getCpuMem(pTag->GetRegisterArea()) == CM_NON)
        return false;
...
/**
 * @brief S7_200::readData
 * @details 从plc设备读一个变量
 * @param pTag 设备变量
 * @return 0-失败,1-成功
 */
int S7_200::readData(IOTag* pTag) {
    // 发送数据结构
    ...
    memcpy(writeDataBuffer_ + 19 + iTemp, &stRDataMsgUnit, iTemp);

    // 记录返回有效数据长度
    wRDataLen = wTemp;
    // 生成超长数据处理延时(经验值)
    if(wRDataLen > 100)
        dwBigDataWaitAdj = wRDataLen - 100;
    ...
    if(getPort() != Q_NULLPTR)
        getPort()->write(writeDataBuffer_, nLen + 21, 1000);
    ...
    if(readDataBuffer_[0] == 0xE5 || readDataBuffer_[0] == 0x10) {
        ...
        do
        {
            ...
            if(getPort() != Q_NULLPTR)
                getPort()->write((quint8 *)&stReadAck, 6, 1000);
        ...
        // 等待plc应答申请数据
        if(getPort() != Q_NULLPTR)
            resultlen = getPort()->read(readDataBuffer_ + 1, 3, 3000);
        ...
        if(getPort() != Q_NULLPTR)
            resultlen = getPort()->read(readDataBuffer_ + 4, (size_t)byTemp + 2, 3000);
        ...
        if(getCpuMem(pTag->GetRegisterArea()) == CM_T) {
            ...
            if(pTag->GetDataType() == TYPE_BOOL) {
                if((*pVailableRecData & 0x02) == 0x02)
                    writeDataBuffer_[0] = 1;
                else
                    writeDataBuffer_[0] = 0;
            } else {
        ...
        // 消息组数据指针移动到下一组消息头
        if((wRDataLen & 0x07) == 0)
            wRDataLen = wRDataLen / 8;

        if((wRDataLen & 0x01) == 0x01)
            wRDataLen += 1;
