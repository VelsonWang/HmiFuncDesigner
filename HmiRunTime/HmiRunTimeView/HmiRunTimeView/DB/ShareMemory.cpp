...
typedef struct tagPersonInfo TPersonInfo;
typedef struct tagPersonInfo *PPersonInfo;
...
int main()
{
    printf("=========testWrite==begin========\n");
    Sharememory mem;
    mem.setName("mem_xbxbxbxb");
    if(mem.create(sizeof(TPersonInfo) * 1000))
    {
        PPersonInfo pBase = (PPersonInfo)mem.open();
        printf("pBase: %p\n", pBase);
        if(NULL != pBase)
        {
            for(int i=0; i< 20; i++) {
                memset(&pBase[i], 0, sizeof(TPersonInfo));
                snprintf(pBase[i].szName, 64, "name_%02d", i);
                snprintf(pBase[i].szSex, 64, "sex_%02d", 20 + i);
                pBase[i].iAge = 40 + i;
            }

            for(int i=0; i< 20; i++) {
                printf("name: %s, sex: %s, age: %d\n", pBase[i].szName, pBase[i].szSex, pBase[i].iAge);
            }
        }
    }
...
int main()
{
    printf("=========testRead==begin========\n");
    Sharememory mem;
    mem.setName("mem_xbxbxbxb");
    if(mem.create(sizeof(TPersonInfo) * 2000))
    {
        printf("hahahaha......\n");
