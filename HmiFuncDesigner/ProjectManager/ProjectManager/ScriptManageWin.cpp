...
QList<ScriptObject *> ScriptFileManage::m_listScriptInfo =
    QList<ScriptObject *>();
...
void ScriptFileManage::save(const QString &filename, SaveFormat saveFormat) {
  ...
  saveFile.write(saveFormat == Json ? saveDoc.toJson()
...
ScriptManageWin::ScriptManageWin(QWidget *parent, QString itemName,
                                 QString projName)
  ...
  connect(m_pListWidget, SIGNAL(itemDoubleClicked(QListWidgetItem *)), this,
...
void ScriptManageWin::open() {
  ...
  QListWidgetItem *pNewItem =
      new QListWidgetItem(QIcon(":/images/pm_script.png"), tr("新建脚本"));
  ...
  for (int i = 0; i < ScriptFileManage::m_listScriptInfo.count(); i++) {
    ...
    QListWidgetItem *pItem =
        new QListWidgetItem(QIcon(":/images/pm_script.png"), scriptName);
...
void ScriptManageWin::NewScript() {
  ...
  ScriptConditionConfigForm *pDlg =
      new ScriptConditionConfigForm(strProjectPath, this);
  ...
  if (pDlg->exec() == QDialog::Accepted) {
    ...
    if (pObj->m_strName != pCurItem->text()) {
      ...
      QString newScriptFileName =
          strProjectPath + "/Scripts/" + pObj->m_strName + ".js";
    ...
    ScriptEditorDlg *pScriptEditorDlg =
        new ScriptEditorDlg(strProjectPath, this);
