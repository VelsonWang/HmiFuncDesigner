...
void WidgetHandle::mouseReleaseEvent(QMouseEvent *e)
{
    ...
    e->accept();
    emit mouseButtonRelease(m_origGeom, m_geom);
}
    ...
    m_formWindow(parent)
{
    for (int i = WidgetHandle::LeftTop; i < WidgetHandle::TypeCount; ++i)
    {
        ...
        connect(m_handles[i],SIGNAL(mouseButtonRelease(QRect,QRect)),this,SLOT(mouseButtonRelease(QRect,QRect)));
    }
    hide();
}
...
void WidgetSelection::mouseButtonRelease(const QRect &old, const QRect &now)
{
