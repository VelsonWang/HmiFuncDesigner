...
int DeviceInfo::m_startNewDeviceID = 0;

DeviceInfo::DeviceInfo()
{
    m_listDeviceInfoObject.clear();
}

DeviceInfo::~DeviceInfo()
{
    qDeleteAll(m_listDeviceInfoObject);
    m_listDeviceInfoObject.clear();
}
...
bool DeviceInfo::openFromXml(XMLObject *pXmlObj)
{
    ...
    foreach(XMLObject* pDevObj, listUsersObj) {
        ...
        m_listDeviceInfoObject.append(pObj);
    }
    return true;
}
...
bool DeviceInfo::saveToXml(XMLObject *pXmlObj)
{
    ...
    for(int i = 0; i < m_listDeviceInfoObject.count(); i++) {
        DeviceInfoObject *pObj = m_listDeviceInfoObject.at(i);
