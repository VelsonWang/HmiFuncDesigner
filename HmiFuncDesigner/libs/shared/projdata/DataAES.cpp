...
int DataAES::Encrypt(QByteArray &input, QByteArray &output, const QString &key)
{
    struct AesCtx ctx;
    Aes::InitAesCtx(&ctx, (const uint8_t*)key.toStdString().c_str());
    ...
    output.append((const char *)header, 16);
    ...
    memset(header, 0, 16);
    // Update Header
    memset(tempBytes, 0, 4);
    DataAES::IntToBytes(line, tempBytes);
    for (size_t i = 0; i < 4; i++) {
        header[i] = tempBytes[i];
    }
    memset(tempBytes, 0, 4);
    DataAES::IntToBytes(offset, tempBytes);
    for (size_t i = 0; i < 4; i++) {
...
int DataAES::Decrypt(QByteArray &input, QByteArray &output, const QString &key)
{
    struct AesCtx ctx;
    Aes::InitAesCtx(&ctx, (const uint8_t*)key.toStdString().c_str());
    ...
    line = DataAES::Byte2ToInt(tempBytes);
    for (size_t i = 0; i < 4; i++) {
        tempBytes[i] = buffer[4 + i];
    }
    offset = DataAES::Byte2ToInt(tempBytes);
...
uint32_t DataAES::Byte2ToInt(uint8_t* bytes)
{
...
void DataAES::IntToBytes(uint32_t i, uint8_t* bytes)
{
